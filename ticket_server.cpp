...
class CorrectnessClient:
    ...
    def do_reservation(self, event_id, ticket_count):
        ...
        if expecting_255:
            ...
            if (event_id == 7):
                print(f"____________________________chce wziac {ticket_count}")
            ...
            if (event_id == 7):
                print(f"cos sobie printuje!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! {self.events[event_id]}")
            print(f"ticket count to {ticket_count} a dla res to {reservation_info.ticket_count}")
            # print(f"event id to {event_id}")
...
def test_big_correctness():
    server = start_server(generate_file(events_various_ticket_count))
    client = CorrectnessClient()
    print(f"printuje teges smieges przed rezerwacjami!:D {client.events[7]}")
    for i in range(7500):
        print(i)
